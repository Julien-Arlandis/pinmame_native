...
static esp_err_t uac_cs_request_get_volume_range(uac_iface_t *iface, int16_t *volume_min_db, int16_t *volume_max_db, int16_t *volume_res_db)
{
    ...
    uint8_t vol_ch_map = iface->iface_alt[iface->cur_alt].vol_ch_map;
    UAC_RETURN_ON_FALSE(feature_unit && vol_ch_map, ESP_ERR_NOT_SUPPORTED, "volume control not supported");
    /* Certains casques (ex. USB-C UAC sans firmware de contrôle de volume) ne
     * répondent pas aux requêtes GET_MIN/GET_MAX/GET_RES et laissent le transfert
     * de contrôle en suspens jusqu'au timeout (5 s).  Ce transfert en vol empêche
     * ensuite la soumission du SET_INTERFACE (uac_host_device_start échoue avec
     * error 268 "Unable to submit control transfer").
     * Solution : retourner des valeurs par défaut sans envoyer de requête USB. */
    *volume_min_db = -12288; /* -48 dB  (48 * 256) */
    *volume_max_db = 0;      /*   0 dB              */
    *volume_res_db = 256;    /*   1 dB par pas      */
    return ESP_OK;
    uint8_t ctrl_iface_num = iface->parent->ctrl_iface_num;
    esp_err_t ret = ESP_OK;
    uint8_t tmp[2] = { 0, 0 };
